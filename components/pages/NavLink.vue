<template>
  <div :class="['NavLink','nav-link-item',{selected}]" @click="onClick">
    <div class="left2">
      <div v-if="inPage" class="angle-left">
      ...
      <svg v-else-if="outPage" :class="{selected}" width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d="M8 8L16 16" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
        <path d="M14 8L8.04286 8.042L8 14" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
      </svg>
    </div>
    <div class="lebel">{{navLabel}}</div>
